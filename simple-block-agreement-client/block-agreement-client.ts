...
async function logToConsole(message: string, type: 'info' | 'success' | 'error' | 'warning' | 'status' = 'info'): Promise<void> {
 ...
 */
async function writeToClient(message: string, type: 'info' | 'success' | 'error' | 'warning' | 'status' = 'info', logToFile: boolean = true): Promise<void> {
    const timestamp = new Date().toLocaleTimeString();
    let prefix = '';
...
async function voteOnTask(task: Task): Promise<void> {
    ...
    const voteMatches = Array.from(updatedContent.matchAll(new RegExp(`VOTE\\|${task.id}\\|S\\d+\\|(\\d+)\\|(\\d+)`, 'g')));
    ...
    if (updatedContent.includes(`TASK_COMPLETE|${task.id}`) || updatedContent.includes(`TASK_EXPIRED|${task.id}`)) {
        return;
    }
