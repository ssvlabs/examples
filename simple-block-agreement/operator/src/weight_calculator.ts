...
export interface WeightCalculator {
    calculateParticipantsWeight(obligatedBalances: Map<token, Map<strategyID, amount>>,
...
export class HarmonicWeightCalculator implements WeightCalculator {
    ...
    public computeHarmonicWeight(
            ...
            strategyValidatorBalanceWeight : number): number {
    ...
    public computeNormalizedHarmonicWeight(
        ...
        normalizationFactor: number): number {
    ...
    public computeWeightNormalizationFactor(
        ...
        strategyValidatorBalanceWeights: Map<strategyID, number>): number {

        var normalizationFactor = 0;

        for (const strategy of strategyTokenWeight.keys()) {
            normalizationFactor += this.computeHarmonicWeight(strategyTokenWeight.get(strategy)!, strategyValidatorBalanceWeights.get(strategy)!);
        }
    ...
    calculateParticipantsWeight(
        ...
        validatorBalances: Map<strategyID, amount>,
        risks: Map<token, Map<strategyID, number>>): Map<strategyID, number> {

        // Normalize token significance
        this.normalizeTokenSignificance();

        // Compute the Map: Strategy -> Token -> Weight
        var strategyTokenWeights = new Map<strategyID, Map<token, number>>();
        for (const token of obligatedBalances.keys()) {

            // Get Strategy -> Weight for the token
            const strategyWeightsForToken = computeStrategyWeights(obligatedBalances.get(token)!, risks.get(token)!, this.beta.get(token)!);
        ...
        var validatorBalanceWeights = computeValidatorBalanceWeights(validatorBalances);

        const normalizationFactor = this.computeWeightNormalizationFactor(strategyTokenWeights, validatorBalanceWeights);


        var ret = new Map<strategyID, number>();
        for (const strategy of strategyTokenWeights.keys()) {
            ret.set(strategy, this.computeNormalizedHarmonicWeight(strategyTokenWeights.get(strategy)!, validatorBalanceWeights.get(strategy)!, normalizationFactor));
        }
...
function calculateNormalizedWeight(amount: number, risk: number, beta: number, totalAmount: amount, normalizationFactor: number): number {
...
function computeStrategyWeights(
    amounts: Map<strategyID, amount>,
    risks: Map<strategyID, number>,
    beta: number): Map<strategyID, number> {
    ...
    for (const strategy of amounts.keys()) {
        strategyWeights.set(strategy, calculateNormalizedWeight(amounts.get(strategy)!, risks.get(strategy)!, beta, totalAmount, normalizationFactor));
    }
...
function computeValidatorBalanceWeights(
    amounts: Map<strategyID, amount>
): Map<strategyID, number> {
