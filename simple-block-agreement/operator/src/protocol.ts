...
export class ParticipantState {
  ...
  public async handleNewBlock(slot: number, hash: string): Promise<void> {
    ...
    const signature = await this.cryptoService.signAsync(vote, this.privateKey)
  ...
  public async processVote(signedVote: SignedVote): Promise<void> {
    ...
    this.log(
      `🗳️ Received vote from ${color}participant ${signedVote.participantID}${this.colorReset()} with slot ${signedVote.vote.slot} and hash ${signedVote.vote.hash}`,
    )
    ...
    for (const [hash, votesForHash] of hashVoteMap.entries()) {
      if (await this.hasMajority(votesForHash)) {
        ...
        if (slot > this.lastDecidedSlot) {
          ...
          this.log(
            `⛓️ Majority found for already decided slot ${slot}. Current highest decided slot: ${this.lastDecidedSlot}`,
          )
        }
        return
      } else {
  ...
  private async hasMajority(signedVotes: SignedVote[]): Promise<boolean> {
    const blockHash = signedVotes[0]?.vote.hash

    this.log(`📄 Checking slot ${signedVotes[0]?.vote.slot} with votes on hash ${blockHash}`)

    // Fetch participants
    const participants = await this.participantFetcher.fetchParticipants(blockHash)
