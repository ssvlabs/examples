...
// Balances
export const strategy1Balance = new Map([[TestingToken1, 1000], [TestingToken2, 2000]]);
export const strategy2Balance = new Map([[TestingToken1, 4000], [TestingToken2, 5000]]);
export const strategy3Balance = new Map([[TestingToken1, 100], [TestingToken2, 100]]);

// Obligations
export const strategy1Obligation = new Map([[TestingToken1, new Map([[TestingBApp, 0.1], [TestingBApp2, 1.0]])],
                                    [TestingToken2, new Map([[TestingBApp, 0.2], [TestingBApp2, 0.5]])]]);
export const strategy2Obligation = new Map([[TestingToken1, new Map([[TestingBApp, 0.3], [TestingBApp2, 1.0]])],
                                    [TestingToken2, new Map([[TestingBApp, 0.4], [TestingBApp2, 0.5]])]]);
...
export class TestingbAppsPlatformAPI implements bAppsPlatformAPI {
    GetbAppTokens(bApp: string): Promise<Map<string, number>> {
        if (bApp === TestingBApp) {
            return Promise.resolve(new Map([[TestingToken1, BetaToken1], [TestingToken2, BetaToken2]]));
        }
    ...
    GetDelegatorsToAccount(account: string): Promise<Map<string, number>> {
        if (account === owner1) {
            return Promise.resolve(new Map([[delegator1, delegator1ToOwner1], [delegator2, delegator2ToOwner1]]));
        }
        if (account === owner2) {
